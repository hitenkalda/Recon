...
export function validatePAN(raw) {
    ...
    if (!PAN_REGEX.test(pan))
        return { valid: false, reason: 'Invalid PAN format (expected AAAPL1234C)' };
    if (!PAN_ENTITY_CHARS.has(pan[3]))
        return { valid: false, reason: `Invalid entity type character '${pan[3]}'` };
    return { valid: true };
}
export function validateGSTIN(raw) {
    const gstin = clean(raw);
    if (!GSTIN_REGEX.test(gstin))
        return { valid: false, reason: 'Invalid GSTIN format (15 chars: state+PAN+entity+Z+check)' };
    // State code validation
    const stateCode = parseInt(gstin.slice(0, 2), 10);
    if (stateCode < 1 || stateCode > 37 || stateCode === 0)
        return { valid: false, reason: `Invalid GSTIN state code '${stateCode}'` };
    // Embedded PAN check
    const panResult = validatePAN(gstin.slice(2, 12));
    if (!panResult.valid)
        return { valid: false, reason: `GSTIN contains invalid PAN: ${panResult.reason}` };
    ...
    if (checkDigit !== gstin[14])
        return { valid: false, reason: `GSTIN check digit mismatch (expected '${checkDigit}', got '${gstin[14]}')` };
    return { valid: true };
}
export function validateTAN(raw) {
    const tan = clean(raw);
    if (!TAN_REGEX.test(tan))
        return { valid: false, reason: 'Invalid TAN format (expected AAAA00000A)' };
    return { valid: true };
}
export function validateCIN(raw) {
    const cin = clean(raw);
    if (!CIN_REGEX.test(cin))
        return { valid: false, reason: 'Invalid CIN format (21 characters expected)' };
    return { valid: true };
}
export function validateIFSC(raw) {
    const ifsc = clean(raw);
    if (!IFSC_REGEX.test(ifsc))
        return { valid: false, reason: 'Invalid IFSC format (expected XXXX0YYYYYY)' };
    return { valid: true };
}
export function validateMobile(raw) {
    const mobile = raw.replace(/[\s\-+91]/g, '');
    if (!MOBILE_REGEX.test(mobile))
        return { valid: false, reason: 'Invalid Indian mobile (expected 10 digits, 6-9 start)' };
    return { valid: true };
}
export function validateAadhaar(raw) {
    const aadhaar = raw.replace(/\s/g, '');
    if (!AADHAAR_REGEX.test(aadhaar))
        return { valid: false, reason: 'Invalid Aadhaar (expected 12 digits)' };
    return { valid: true };
}
export function validatePincode(raw) {
    const pin = raw.trim();
    if (!PINCODE_REGEX.test(pin))
        return { valid: false, reason: 'Invalid pin code (expected 6 digits)' };
    return { valid: true };
}
...
export function gstStateName(code) {
