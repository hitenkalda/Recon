...
type PlanTab = 'firms' | 'solo' | 'enterprise' | 'individual';
...
const INDIVIDUAL_PLANS = [
  {
    ...
    price: '₹0',
    period: '',
    sub: 'For individuals getting started with GST, bank, and AIS/26AS reconciliation.',
    features: [
      { text: '5 reconciliation jobs / day', highlight: true },
      'GST, Bank & AIS/26AS matching',
      'CSV & Excel report export',
      'Single file upload',
      'Community support',
    ],
    cta: { label: 'Get Started Free', href: '/individual/signup' },
    featured: false,
  },
  {
    name: 'Basic',
    ...
    sub: 'For practicing individuals who need higher daily throughput and more report formats.',
    features: [
      { text: '30 reconciliation jobs / day', highlight: true },
      'All report formats (PDF, Excel, CSV)',
      'Multi-file upload per job',
      'Priority queue processing',
      'Email support',
    ],
    cta: { label: 'Start Basic Plan', href: '/individual/signup' },
    featured: true,
  },
  {
    name: 'Pro',
    ...
    period: '/ month',
    billed: 'Billed monthly',
    sub: 'For serious practitioners requiring unlimited daily reconciliations and advanced analytics.',
    features: [
      { text: 'Unlimited reconciliation jobs', highlight: true },
      'All Basic features included',
      'Advanced discrepancy analytics',
      'Unlimited document uploads',
      'Dedicated support channel',
    ],
    cta: { label: 'Start Pro Plan', href: '/individual/signup' },
    featured: false,
  },
];
...
export default function PricingPage() {
  ...
  const showIndividual = tab === 'individual';
  const plans = showIndividual ? INDIVIDUAL_PLANS : PLANS;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-[#07090e]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group" title="ReconAI">
            <Brand />
          </Link>
          ...
          <div className="flex items-center gap-4">
            <Link href={showIndividual ? '/individual/login' : '/login'} className="text-xs font-medium text-slate-300 hover:text-white transition-colors hidden sm:block">
              Sign In
            </Link>
            <Link
              href={showIndividual ? '/individual/signup' : '/signup'}
              className="px-4 py-2 rounded-full text-xs font-semibold bg-white text-slate-950 hover:bg-cyan-300 hover:shadow-[0_0_45px_-10px_rgba(0,240,255,0.22)] transition-all duration-300 flex items-center gap-1.5 shadow-md"
            >
              {showIndividual ? 'Start Free' : 'Start Free Trial'}
            </Link>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 flex-1">
        <section className="pt-16 pb-12 text-center max-w-4xl mx-auto px-4 sm:px-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-cyan-500/25 bg-cyan-950/20 text-cyan-300 text-xs font-medium mb-6 backdrop-blur-md">
            ...
            {showIndividual ? 'Personal Tax & Ledger Recon' : 'Firm-Ready Audit Intelligence'}
          </div>
          <h1
          ...
          <p className="text-slate-400 text-base sm:text-lg max-w-xl mx-auto mb-10 font-normal">
            {showIndividual
              ? 'Start free. Reconcile your own GST, bank, and AIS documents — no firm required.'
        ...
        {/* ── Feature comparison (only for firms/enterprise tabs) ────── */}
        {!showIndividual ? (
          ...
          <section className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
            <div className="text-center mb-8">
              <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight text-white mb-2" style={{ fontFamily: 'var(--font-geist)' }}>
                What&apos;s included
              </h2>
              <p className="text-xs sm:text-sm text-slate-400">
                Every Individual plan includes GST, Bank, and AIS/26AS reconciliation with secure document storage.
              </p>
            </div>
            <div className="overflow-x-auto rounded-2xl border border-slate-800/90 bg-[#090e18]/80 backdrop-blur-md shadow-2xl">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-slate-800 bg-slate-950/70">
                    <th className="py-5 px-6 font-medium text-slate-400 w-1/3" scope="col">Feature</th>
                    <th className="py-5 px-4 text-center font-semibold text-white w-1/3" scope="col">
                      <div className="text-sm font-semibold mb-1">Free</div>
                      <Link href="/individual/signup" className="inline-block px-3 py-1 text-[11px] font-medium rounded-full border border-slate-700 hover:border-slate-500 bg-slate-900 text-slate-300">
                        Get Started
                      </Link>
                    </th>
                    <th className="py-5 px-4 text-center font-semibold text-white w-1/3 bg-cyan-950/20 border-x border-cyan-500/20" scope="col">
                      <div className="text-sm font-semibold text-cyan-300 mb-1">Basic / Pro</div>
                      <Link href="/individual/signup" className="inline-block px-3.5 py-1 text-[11px] font-semibold rounded-full bg-cyan-400 text-slate-950 hover:bg-cyan-300 shadow-sm">
                        Upgrade
                      </Link>
                    </th>
                  </tr>
                ...
                <tbody className="divide-y divide-slate-800/60 font-light">
                  {[
                    { label: 'Daily Reconciliation Jobs', free: '5', paid: '30 / unlimited' },
                    { label: 'Report Formats', free: 'CSV & Excel', paid: 'All (PDF, Excel, CSV)' },
                    { label: 'Multi-file Upload', free: '—', paid: '✓' },
                    { label: 'Processing Priority', free: 'Standard queue', paid: 'Priority / Dedicated' },
                    { label: 'Support', free: 'Community', paid: 'Email / Dedicated' },
                    { label: 'Document Storage', free: '30 days', paid: 'Unlimited' },
                  ].map((row) => (
                    <tr key={row.label} className="hover:bg-white/[0.02] transition-colors">
                      <td className="py-3.5 px-6 text-slate-300 font-medium">{row.label}</td>
                      <td className="py-3.5 px-4 text-center text-slate-400">{row.free}</td>
                      <td className="py-3.5 px-4 text-center text-cyan-400 font-bold bg-cyan-950/10 border-x border-cyan-500/10">{row.paid}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* ── Enterprise CTA banner (firms only) ────────────────────── */}
        {!showIndividual ? (
