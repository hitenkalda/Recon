...
function SignupForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await api.post('/individual/auth/signup', {
        name: form.name,
        email: form.email,
        ...
        >
          ...
          <div className="relative z-10 p-6 sm:p-8 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[13px] text-zinc-400 leading-relaxed">Start reconciling documents today — free up to 5 jobs/day</p>
            </div>
            ...
            <form onSubmit={submit} className="flex flex-col gap-3.5">
              <Field label="Full Name" required>
                <Input value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="Your name" required minLength={2} autoComplete="name" />
              </Field>
            ...
            <div className="pt-1 text-center">
              <span className="text-[12px] text-zinc-400">Already have an account? </span>
              <Link className="text-[12px] font-medium text-cyan-300 hover:text-cyan-200 underline underline-offset-4 decoration-cyan-400/40 transition-all" href="/individual/login">
                Sign in
              </Link>
            </div>
