...
export function createApp() {
  ...
  app.use((req, _res, next) => {
    ...
    (req as unknown as { log: ReturnType<typeof logger.child> }).log = logger.child({ requestId: req.id });
    next();
  });
  app.use(pinoHttp({
    logger,
    genReqId: (req) => (req as { id?: string }).id ?? nanoid(12),
    customLogLevel: (_req, res, err) => (err || res.statusCode >= 500 ? 'error' : 'info'),
    autoLogging: { ignore: (req) => req.url === '/api/health' },
    serializers: {
      req: (r) => ({ id: (r as { id?: unknown }).id, method: r.method, url: r.url }),
    },
  }));
  ...
  app.use(((err: unknown, req: Request, res: Response, _next: NextFunction) => {
    const reqLogger = (req as unknown as { log?: ReturnType<typeof logger.child> }).log ?? logger;
