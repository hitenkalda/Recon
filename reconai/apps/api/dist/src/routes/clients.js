...
router.get('/', requireCapability('clients.read'), asyncHandler(async (req, res) => {
    ...
    const where = { firmId: me.firmId, isDeleted: false };
    if (q.status)
    ...
    if (q.filter === 'active_gst')
        where.gstin = { not: null };
    if (q.filter === 'tax_audit')
        where.tags = { array_contains: ['tax_audit'] };
...
function assertUniqueIdentifiers(input) {
    if (input.gstin && !validateGSTIN(input.gstin).valid)
        throw BadRequest(validateGSTIN(input.gstin).reason ?? 'Invalid GSTIN', { gstin: input.gstin });
    if (input.pan && !validatePAN(input.pan).valid)
        throw BadRequest(validatePAN(input.pan).reason ?? 'Invalid PAN', { pan: input.pan });
    if (input.tan && !validateTAN(input.tan).valid)
        throw BadRequest(validateTAN(input.tan).reason ?? 'Invalid TAN', { tan: input.tan });
    if (input.cin && !validateCIN(input.cin).valid)
        throw BadRequest(validateCIN(input.cin).reason ?? 'Invalid CIN', { cin: input.cin });
}
