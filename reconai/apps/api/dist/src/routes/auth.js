...
router.post('/login', asyncHandler(async (req, res) => {
    ...
    if (!membership)
        throw Forbidden('This account is not attached to a firm');
    if (!membership.firm.isActive)
        throw Forbidden('This firm has been deactivated');
    await prisma.user.update({ where: { id: user.id }, data: { lastLoginAt: new Date() } });
    logAudit(req, { action: 'auth', entityType: 'session', detail: { kind: 'login' } });
    res.cookie(cookieName, signToken(user.id, membership.firmId, membership.role), cookieOptions);
...
router.post('/invite', requireAuth, requireCapability('team.manage'), asyncHandler(async (req, res) => {
    ...
    const existingMember = existing
        ...
        : null;
    if (existingMember)
        throw Conflict('This person is already a member of your firm');
    if (input.role === Role.FirmAdmin)
        throw Forbidden('Admins must be provisioned by the firm root account');
...
router.post('/can', requireAuth, asyncHandler(async (req, res) => {
    ...
    res.json({ allowed: hasCapability(me.role, cap) });
}));
