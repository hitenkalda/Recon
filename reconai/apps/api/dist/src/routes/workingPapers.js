...
router.patch('/:id', requireCapability('workingpapers.create'), asyncHandler(async (req, res) => {
    ...
    if (input.status === 'senior_signed' && wp.status !== WorkingPaperStatus.PartnerSigned) {
        if (!['senior', 'partner', 'firm_admin'].includes(me.role))
            throw Forbidden('Only seniors and partners can mark review complete');
    ...
    if (input.status === 'partner_signed') {
        if (!['partner', 'firm_admin'].includes(me.role))
            throw Forbidden('Only partners can sign off working papers');
        const content = {
...
//# sourceMappingURL=workingPapers.js.map