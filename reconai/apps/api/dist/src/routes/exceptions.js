...
import { exceptionCreateSchema, exceptionUpdateSchema } from '@reconai/shared';
...
router.get('/:id', requireCapability('exceptions.read'), asyncHandler(async (req, res) => {
    ...
    if (exception.assigneeId)
        wanted.add(exception.assigneeId);
    for (const c of exception.comments)
        wanted.add(c.userId);
    for (const a of exception.activities)
        if (a.userId)
            wanted.add(a.userId);
...
router.post('/', requireCapability('exceptions.resolve'), asyncHandler(async (req, res) => {
    ...
    if (input.referenceItemIds.length && run) {
        ...
        if (cnt !== input.referenceItemIds.length)
            throw BadRequest('Some referenced items do not belong to this run');
    }
    const exception = await prisma.exception.create({
        data: {
            firmId: me.firmId,
            runId: run?.id ?? null,
            engagementId: req.body.engagementId ?? run?.engagementId ?? '',
            clientId: req.body.clientId ?? run?.clientId ?? '',
            kind: input.kind,
            ...
            references: input.referenceItemIds,
        },
    });
...
router.post('/:id/comments', requireCapability('exceptions.read'), asyncHandler(async (req, res) => {
    ...
    if (!text)
        throw BadRequest('Comment text required');
...
router.post('/:id/evidence', requireCapability('exceptions.resolve'), asyncHandler(async (req, res) => {
    ...
    if (documentId) {
        ...
        if (!doc)
            throw NotFound('Evidence document not found');
...
router.patch('/:id', requireCapability('exceptions.resolve'), asyncHandler(async (req, res) => {
    ...
    if (input.status && input.status !== exception.status) {
        ...
        if (input.status === 'resolved') {
            if (!canResolveSeverity(me.role, exception.severity)) {
                throw Forbidden('High/critical exceptions require a Senior or above to resolve');
            }
            if (!input.resolution)
                throw BadRequest('Resolution note is required to close an exception');
    ...
    if (events.length) {
        await prisma.exceptionActivity.createMany({
            data: events.map((e) => ({ exceptionId: exception.id, userId: me.userId, action: String(e.action), fromStatus: e.fromStatus ?? undefined, toStatus: e.toStatus ?? undefined, detail: e.detail })),
        });
    }
...
async function usersById(ids) {
    const clean = [...new Set(ids.filter((x) => Boolean(x)))];
    if (!clean.length)
        return new Map();
...
function validateTransition(from, to) {
    ...
    if (!(allowed[from] ?? []).includes(to))
        throw BadRequest(`Cannot move an exception from ${from} to ${to}`);
}
