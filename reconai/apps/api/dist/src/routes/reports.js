...
router.post('/generate', requireCapability('reports.generate'), asyncHandler(async (req, res) => {
    ...
    const body = format === ReportFormat.CSV ? toCsv([payload]) : JSON.stringify(payload, null, 2);
...
router.get('/run/:runId/csv', requireCapability('reports.read'), asyncHandler(async (req, res) => {
    ...
    const rows = items.map((i) => ({
        ...
        score: i.score,
        a: i.recordA?.data ? JSON.stringify((i.recordA?.data ?? {})) : '',
        b: i.recordB?.data ? JSON.stringify((i.recordB?.data ?? {})) : '',
...
async function resolveReportScope(firmId, input) {
    if (input.runId) {
    ...
    if (input.workingPaperId) {
        const wp = await prisma.workingPaper.findFirst({ where: { id: input.workingPaperId, firmId }, include: { run: { select: { id: true, type: true, metrics: true } }, client: { select: { name: true } } } });
        if (!wp)
            throw NotFound('Working paper not found');
        return {
