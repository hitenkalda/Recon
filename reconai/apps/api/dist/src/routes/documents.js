...
import { uploadInitSchema, documentConfirmSchema, reprocessSchema, columnMappingSchema, documentQuerySchema, ProcessingStage, } from '@reconai/shared';
...
router.post('/upload-init', requireCapability('documents.upload'), asyncHandler(async (req, res) => {
    ...
    if (overSize)
        throw BadRequest(`File exceeds the ${config.fileLimits.maxSizeMb} MB limit`, { file: overSize.name });
...
router.get('/', requireCapability('documents.read'), asyncHandler(async (req, res) => {
    ...
    const where = { firmId: me.firmId, status: { not: 'archived' } };
    if (q.engagementId)
    ...
    if (q.search)
        where.originalName = { contains: q.search, mode: 'insensitive' };
...
router.post('/:id/mapping', requireCapability('documents.reprocess'), asyncHandler(async (req, res) => {
    ...
    if (!input.mappings.length)
        throw BadRequest('Provide at least one column mapping');
    const mappings = {};
    for (const m of input.mappings)
        mappings[m.sourceColumn] = m.targetField;
    await prisma.document.update({ where: { id: doc.id }, data: { mapping: mappings, status: ProcessingStage.ColumnsMapped } });
    await prisma.sourceRecord.deleteMany({ where: { documentId: doc.id } });
